...
def get_time_series_for_site(client: CogniteClient, site):
    ...
    sub_tree_root_path = getattr(sub_tree_root, "path", None)
    if not sub_tree_root_path:
        print(
            f"----No asset path available for {site}!----\n"
            f"    Ensure the Cognite asset hierarchy has been created and the CDF root asset has a valid path."
        )
        return []

    sub_tree_nodes = client.data_modeling.instances.list(
        instance_type=CogniteAsset,
        filter=Prefix(property=["cdf_cdm", "CogniteAsset/v1", "path"], value=sub_tree_root_path),
        limit=None
    )
...
def handle(client: CogniteClient = None, data=None):
    ...
    hours = max_hours if hours is None else hours
